...
def main():
    ...
    color_mode = 'blue'
    tool = 'brush'
    drawings = []# сюда сохраняются ВСЕ нарисованные объекты (чтобы они не исчезали)
    current_stroke = None # текущая линия кисти (когда рисуем мышкой)
    shape_start = None  # точка, где начинается фигура
    ...
    while True:
        # получаем состояние клавиш (нажаты ли они сейчас)
        ...
        for event in pygame.event.get():
            ...
            # если пользователь нажал крестик — выходим из программы
            if event.type == pygame.QUIT:
                return
            if event.type == pygame.KEYDOWN:
                # горячие клавиши выхода (Ctrl+W, Alt+F4, Esc)
                ...
                if event.key == pygame.K_UP:
                    radius = min(200, radius + 1)# увеличиваем, но не больше 200
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:  # Left mouse button
                    ...
                    # если выбрана кисть или ластик — рисуем линию
                    if tool == 'brush' or tool == 'eraser':
                        ...
                        # если выбран инструмент фигуры — запоминаем стартовую точку
                        shape_start = event.pos
                    drawing = True

            if event.type == pygame.MOUSEMOTION and drawing:
                current_mouse_pos = event.pos
                # если рисуем кистью — добавляем точки в линию
                if current_stroke is not None:
                    current_stroke["points"].append(event.pos)

            if event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1:
                    # если рисовали фигуру — сохраняем её
        ...
        screen.fill((0, 0, 0))
        # каждый кадр начинаем с чистого фона
        ...
        # показывает фигуру до того, как ты отпустишь мышку
        ...
        # рисуем интерфейс (панель с кнопками и подсказками)
...
def get_base_color(color_mode):
     # возвращает основной цвет в зависимости от выбранного режима
    if color_mode == 'blue':
        return (0, 0, 255)
    elif color_mode == 'red':
        return (255, 0, 0)
...
def draw_ui(screen, tool, color_mode, radius):
    # рисует панель управления сверху
